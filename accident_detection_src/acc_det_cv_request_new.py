...
def detect_changes(frame1, frame2, backend_url, change_threshold):
...
while True:
    ...
    if current_time-time_init>=delay:
    	...
    	result_frame,det = detect_changes(f1, next_frame,backend_url, ch_thr)
    	t2=time.time_ns()//1000000
    	print("Visual processing time:",t2-t1,"ms")
    if det:
    	det=0
    	buzzer.on()
    	buzzer_on=1
